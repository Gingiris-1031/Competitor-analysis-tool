...
function renderStrategyRadar(radar) {
    ...
    const centerScore = `
        ...
        <text x="${cx}" y="${cy + 14}" text-anchor="middle" fill="#6b7280" font-size="10">Evidence Score</text>
    `;
    ...
    let html = `<div class="bg-gray-900 rounded-xl border border-gray-800 p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[10px] bg-white/70 text-[color:var(--accent)] border border-[color:var(--warm-border)] px-2.5 py-1 rounded-full font-medium">${coverage.total ? `${coverage.available}/${coverage.total} evidence groups` : 'Directional benchmark'}</span>
        </div>
    ...
    html += `</div></div>
        <details class="mt-5 border-t border-[color:var(--warm-border)] pt-3">
            <summary class="cursor-pointer text-xs text-[color:var(--accent)]">How this score is calculated</summary>
            <p class="mt-2 text-xs leading-relaxed text-[color:var(--ink-muted)]">${esc(radar.methodology || 'Directional benchmark based on public evidence. Missing data is scored conservatively.')}</p>
        </details>
    </div>`;
