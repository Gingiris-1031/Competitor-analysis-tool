/**
 * render-strategy.js — Custom growth strategy Playbook recommendation render module
 */
...
function renderStrategy(gs) {
    ...
    const productName = gs.product_name || 'competitor';

    let html = `<div class="bg-gray-900 rounded-xl border border-emerald-700/30 p-6">
        <div class="flex items-center justify-between mb-1">
            <h3 class="text-lg font-semibold">📋 Growth Strategy</h3>
            <span class="text-[10px] bg-emerald-900/40 text-emerald-300 border border-emerald-700/40 px-2.5 py-1 rounded-full font-medium">💡 Gingiris Playbook Exclusive</span>
        </div>
        <p class="text-xs text-gray-500 mb-2">Cross-analysis of Wayback history + PH Launch records + social propagation data, auto-matching the best growth Playbook</p>
        <p class="text-[10px] text-emerald-400/60 mb-6">⚡ These custom recommendations combine multiple exclusive data sources for depth analysis beyond what GPT/Claude chat can provide</p>`;
    ...
    if (secondary.length > 0) {
        html += `<div class="mt-4">
            <h4 class="text-xs font-semibold text-gray-400 mb-3">Secondary Recommendations</h4>
    ...
    if (Object.keys(allScores).length) {
        ...
        html += `<div class="mt-4 pt-4 border-t border-gray-800">
            <div class="text-[10px] text-gray-600 mb-1">Match Score Details</div>
...
function _renderPrimaryCard(pb, productName) {
    ...
    return `<div class="bg-gray-800 rounded-xl border border-emerald-500/30 p-5">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] bg-emerald-600 text-white rounded-full px-2 py-0.5 font-medium">Primary</span>
        ...
        ${reasonsHtml ? `<div class="mb-4 bg-gray-900/50 rounded-lg p-3">
            <div class="text-[10px] text-emerald-400/80 font-medium mb-2">📌 Why this Playbook (based on actual competitor data)</div>
            <ul class="space-y-1.5 text-xs text-gray-300">${reasonsHtml}</ul>
        </div>` : ''}

        ${tipsHtml ? `<div>
            <div class="text-[10px] text-emerald-400/80 font-medium mb-2">🎯 Custom action tips based on ${esc(productName)} data</div>
...
function _renderSecondaryCard(pb) {
    ...
    return `<div class="bg-gray-800 rounded-lg border border-gray-700 p-4">
        <div class="flex items-start justify-between mb-2">
            ...
            <a href="${esc(pb.url)}" target="_blank" rel="noopener"
               class="text-[10px] text-emerald-400 hover:text-emerald-300 shrink-0">View →</a>
        </div>
...
function _renderToolsCard(gt) {
    return `<div class="mt-4 flex items-center justify-between bg-gray-800/50 border border-gray-700 rounded-lg px-4 py-3">
        ...
        <a href="${esc(gt.url)}" target="_blank" rel="noopener"
           class="text-[10px] text-gray-400 hover:text-gray-200 shrink-0">View Tools →</a>
    </div>`;
}
