function renderSocial(sm) {
    ...
    if (reddit && reddit.top_posts && reddit.top_posts.length) {
        ...
        if (reddit.mentioned_subreddits && reddit.mentioned_subreddits.length) {
            html += `<div class="mb-6"><h4 class="text-sm font-semibold text-gray-300 mb-2">提及的 Subreddits</h4>
    ...
    html += `<div class="mb-6"><h4 class="text-sm font-semibold text-gray-300 mb-3">📊 传播指标总览</h4>
        <div class="overflow-x-auto"><table class="w-full text-xs">
            ...
            <tbody>
                <tr class="border-t border-gray-800"><td class="px-3 py-2 text-gray-300">总参与人数</td><td class="px-3 py-2 font-mono">${(pm.total_participants||0).toLocaleString()}</td><td class="px-3 py-2 text-gray-500">Followers + Subscribers + Members</td></tr>
                <tr class="border-t border-gray-800"><td class="px-3 py-2 text-gray-300">预估曝光量</td><td class="px-3 py-2 font-mono">${(pm.estimated_impressions||0).toLocaleString()}</td><td class="px-3 py-2 text-gray-500">保守估计（基于粉丝数）</td></tr>
                <tr class="border-t border-gray-800"><td class="px-3 py-2 text-gray-300">总互动量</td><td class="px-3 py-2 font-mono">${(pm.total_engagement||0).toLocaleString()}</td><td class="px-3 py-2 text-gray-500">Likes + Retweets + Upvotes + Stars</td></tr>
                <tr class="border-t border-gray-800"><td class="px-3 py-2 text-gray-300">数据来源</td><td class="px-3 py-2" colspan="2">${(pm.data_sources||[]).join(', ')||'暂无'}</td></tr>
            </tbody>
        </table></div>
    ...
    if (tw && tw.detected && tw.profile && tw.profile.name) {
        html += `<div class="mb-6"><h4 class="text-sm font-semibold text-gray-300 mb-2">📊 Twitter 账号概览</h4>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <div class="bg-gray-800 rounded p-2"><div class="text-[10px] text-gray-400">推文数</div><div class="text-sm font-mono">${tw.profile.statuses_count ? Number(tw.profile.statuses_count).toLocaleString() : '?'}</div></div>
    ...
    if (tw && !tw.detected && tw.key_posts_framework) {
        html += `<div class="mb-4"><h4 class="text-sm font-semibold text-gray-300 mb-2">🔍 需手动补充的数据</h4>
            <div class="bg-yellow-900/10 border border-yellow-800/30 rounded-lg p-3">
                <div class="text-xs text-yellow-300 mb-2">以下数据需要 Twitter API 或手动采集：</div>
                <ul class="space-y-1">`;
