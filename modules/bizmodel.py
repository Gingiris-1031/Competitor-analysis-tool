...
def _classify_model(p: dict, g: dict, ph: dict) -> dict:
    ...
    if has_open_source and stars > 0:
        if "AGPL" in license_ or "GPL" in license_:
            primary.append("OSS-led / Open Core（AGPL，SaaS 需商业授权）")
        elif "MIT" in license_ or "Apache" in license_:
            primary.append("OSS-led（宽松协议，云托管版/企业版变现）")
        else:
            primary.append("OSS-led（开源驱动增长）")

    if has_free and has_paid_tiers:
        primary.append("PLG — Freemium（免费→付费升级）")
    elif has_paid_tiers and p.get("found"):
        secondary.append("Self-serve SaaS")

    if has_enterprise:
        secondary.append("SLG（企业版 + 销售团队）")

    if not primary and not secondary:
        if ph.get("found"):
            primary = ["PLG（社区驱动，Product Hunt 验证）"]
        else:
            primary = ["商业模式待确认（无公开定价数据）"]
...
def _assess_stage(p: dict, t: dict, f: dict, g: dict) -> str:
    ...
    if monthly_visits > 500_000 or stars > 30_000:
        return "规模扩张期（PMF 已验证，商业化扩张）"
    if monthly_visits > 100_000 or stars > 10_000:
        if has_paid:
            return "PMF 验证期（流量稳健，商业化落地中）"
        return "增长期（高流量/Star，商业化待启动）"
    if monthly_visits > 20_000 or stars > 1_000:
        if has_paid or has_series:
            return "早期商业化期（有付费用户/机构融资）"
        return "早期增长期（有用户基础，商业模式探索）"
    return "0→1 阶段（产品早期，建立用户基础）"
...
def _identify_levers(p: dict, g: dict) -> list:
    ...
    if has_free and paid:
        name, price = paid[0]
        levers.append(f"Freemium 升级：免费 → {name}（${price}/月）")

    if any("enterprise" in (t.get("name") or "").lower() for t in tiers):
        levers.append("Enterprise 合同（年付 + 私有部署 + SLA）")

    if g.get("found"):
        if "AGPL" in license_ or "GPL" in license_:
            levers.append("开源协议商业授权（SaaS/私有部署需购买 Commercial License）")
        levers.append("云托管版（SaaS，降低自部署门槛，提升转化）")

    if p.get("annual_discount"):
        levers.append(f"年付折扣锁定用户：{p['annual_discount']}")

    if p.get("free_trial"):
        levers.append("免费试用期（降低付费决策门槛）")

    return levers or ["暂无明确变现杠杆数据"]
...
def _gen_insights(model_type, revenue, stage, p, g, f, ph) -> list:
    ...
    if primary:
        insights.append(f"🏗 商业模式：{' + '.join(primary)}")
    secondary = model_type.get("secondary") or []
    if secondary:
        insights.append(f"➕ 辅助模式：{' / '.join(secondary)}")

    insights.append(f"📊 商业化阶段：**{stage}**")

    arr_low  = revenue.get("arr_low")
    arr_high = revenue.get("arr_high")
    if arr_low and arr_high:
        insights.append(
            f"💰 保守 ARR 估算：**{_fmt_money(arr_low)} – {_fmt_money(arr_high)}**"
            f"（{revenue.get('method', '')}）"
        )

    total_raised = f.get("total_raised")
    if f.get("found") and total_raised:
        insights.append(f"💵 已融资 {_fmt_money(total_raised)}（{f.get('latest_round', '')}）")
    elif f.get("found"):
        insights.append(f"💵 已完成 {f.get('latest_round', '')} 融资（金额未公开）")
    ...
    if stars > 1_000 and forks:
        ...
        if fork_ratio > 0.08:
            insights.append(f"🍴 Fork 率 {fork_ratio:.0%}（{forks:,} forks），社区二次开发活跃")
    ...
    if ph.get("found"):
        ...
        if launch_count > 1:
            insights.append(f"🚀 {launch_count} 次 Product Hunt 发布，最高 {votes:,} votes（PLG 社区验证）")
