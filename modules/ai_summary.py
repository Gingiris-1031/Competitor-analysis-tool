...
**拐点根因三层归因（必写，如 FACTS 里有 traffic_peaks / 流量峰值数据）**：挑出最大的那个增长拐点（哪个月、增幅多少），做三层归因，从结果层层往下挖到根因：
- **表层（战术结果）**：那次流量/指标发生了什么（具体数字 + 时间）。
- **中层（执行动作）**：团队做了什么动作触发的（哪个渠道战役 / 发布 / 机制上线）。
- **深层（战略与人）**：更根本的原因是什么（团队基因 / 信息差 / 定位选择 / 时机）——这一层是"别人抄不走"的判断。
若无峰值数据，写"未捕捉到显著拐点，增长呈线性/自然积累"，不要编。

### 四、增长循环（Growth Loop）

用 **Acquisition → Activation → Retention/Monetization → Referral** 四段拆解这个产品的增长正循环，每段一句话、必须是这个产品**特有**的机制（不套通用模板）：
- **Acquisition（获客）**：新用户从哪来、被什么钩住。
- **Activation（激活）**：怎么完成第一次"aha 时刻"。
- **Retention / Monetization（留存/变现）**：怎么留住、在哪一步变现。
- **Referral（传播）**：用户为什么、通过什么把下一批人带进来（形成闭环回到获客）。
最后一句点明这个循环是「激励型病毒循环 / 内容飞轮 / 开发者生态 / 销售驱动」中的哪一类。
